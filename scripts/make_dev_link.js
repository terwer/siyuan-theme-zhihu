...
if (targetDir === '') {
    ...
    if (res === null || res === undefined || res.length === 0) {
        ...
        if (env !== undefined && env !== null && env !== '') {
            targetDir = env;
            log(`\tGot target directory from environment variable "SIYUAN_THEME_DIR": ${targetDir}`);
        } else {
            error('\tCan not get SiYuan directory from environment variable "SIYUAN_THEME_DIR", failed!');
            process.exit(1);
        }
    } else {
...
if (!name || name === '') {
    error('Failed! Please set theme name in theme.json');
    process.exit(1);
}
